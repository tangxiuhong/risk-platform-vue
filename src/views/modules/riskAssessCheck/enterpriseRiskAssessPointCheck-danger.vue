<template>
  <el-dialog
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="dataFormSubmit(1)" :disabled="this.dataForm.status==1">确定</el-button>
    </span>
  </el-dialog>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      init (riskPointId, riskUnitId, troubleId, checkList) {
        this.resetForm('dataForm')
        this.dataForm.riskPoint = riskPointId
        this.dataForm.riskUnit = riskUnitId
        this.dataForm.troubleId = troubleId
        this.dataForm.id = troubleId
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              ...
              if (data && data.code === 0) {
                this.$http({
                  url: this.$http.adornUrl(`/trouble/enterpriseTroublerecord/save`),
                  method: 'post',
                  data: this.$http.adornData({
                    'id': this.dataForm.troubleId || undefined,
