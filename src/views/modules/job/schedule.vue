  ...
  <div class="mod-schedule">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('sys/schedule/save')" type="primary" @click="addOrUpdateHandle(0)">新增</el-button>
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-tag v-if="scope.row.status === 'NORMAL'" size="small">正常</el-tag>
          <el-tag v-if="scope.row.status === 'PAUSED'" size="small">暂停</el-tag>
          <el-tag v-if="scope.row.status === 'COMPLETE'" size="small">完成</el-tag>
          <el-tag v-if="scope.row.status === 'ERROR'" size="small" type="danger">错误</el-tag>
          <el-tag v-if="scope.row.status === 'BLOCKED'" size="small" type="danger">阻塞</el-tag>
          <el-tag v-if="scope.row.status === 'NONE'" size="small" type="danger">不存在</el-tag>
        </template>
      </el-table-column>
      <el-table-column
        fixed="right"
        ...
        <template slot-scope="scope">
          <el-button v-if="isAuth('sys/schedule/update')" type="text" size="small"
                     @click="addOrUpdateHandle(1,scope.row.jobName,scope.row.jobGroup)">修改
          </el-button>
          <el-button v-if="isAuth('sys/schedule/delete')" type="text" size="small"
                     @click="deleteHandle(scope.row.jobName,scope.row.jobGroup)">删除
          </el-button>
          <el-button v-if="isAuth('sys/schedule/pause')" type="text" size="small"
                     @click="pauseHandle(scope.row.jobName,scope.row.jobGroup,scope.row.jobGroup)">暂停
          </el-button>
          <el-button v-if="isAuth('sys/schedule/resume')" type="text" size="small"
                     @click="resumeHandle(scope.row.jobName,scope.row.jobGroup,scope.row.jobGroup)">恢复
          </el-button>
          <el-button v-if="isAuth('sys/schedule/run')" type="text" size="small"
                     @click="runHandle(scope.row.jobName,scope.row.jobGroup)">立即执行
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 新增 / 修改
      addOrUpdateHandle (id, jobName, jobGroup) {
        this.addOrUpdateVisible = true
        this.$nextTick(() => {
          this.$refs.addOrUpdate.init(id, jobName, jobGroup)
        })
      },
      // 删除
      deleteHandle (jobName, jobGroup) {
        this.$confirm(`确定对[id=${jobName}]进行[${jobName ? '删除' : '批量删除'}]操作?`, '提示', {
      ...
      pauseHandle (jobName, jobGroup) {
        this.$confirm(`确定对[id=${jobName}]进行[${jobName ? '暂停' : '批量暂停'}]操作?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
      ...
      resumeHandle (jobName, jobGroup) {
        this.$confirm(`确定对[id=${jobName}]进行[${jobName ? '恢复' : '批量恢复'}]操作?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
      ...
      runHandle (jobName, jobGroup) {
        this.$confirm(`确定对[id=${jobName}]进行[${jobName ? '立即执行' : '批量立即执行'}]操作?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
