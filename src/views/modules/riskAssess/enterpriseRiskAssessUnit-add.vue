  ...
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()"
     ...
     <!-- <el-form-item label="评估编号" prop="assessNo">
        <el-select v-model="dataForm.assessNo" placeholder="评估编号">
      ...
      <el-form-item label="单元确认人" prop="listPerson">
        <el-select v-model="dataForm.listPerson"  filterable placeholder="单元确认人">
          <el-option
      ...
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            ...
            this.$http({
              url: this.$http.adornUrl(`/riskAssess/enterpriseRiskAssessResult/listDefaultUsers`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.listPerson = data.list.userId;
                this.dataForm.listDate = data.list.time;
              }
            })
          }
        })
        ...
        this.$http({
          url: this.$http.adornUrl(`/riskAssess/enterpriseRiskAssessResult/listSelectAssessNo`),
          method: 'post',
          params: this.$http.adornParams()
        }).then(({data}) => {
          if (data && data.code === 0) {
