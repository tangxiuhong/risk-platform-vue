      ...
      <el-row>
        <el-col :span="10">
          <el-form-item label="发生的可能性" prop="riskProbability">
            <el-select v-model="dataForm.riskProbability" placeholder="发生的可能性" @change="riskProbabilityChange">
              <el-option
        ...
        <el-col :span="10">
          <el-form-item label="后果严重性" prop="riskSeverity">
            <el-select v-model="dataForm.riskSeverity" placeholder="后果严重性" @change="riskSeverityChange">
              <el-option
      ...
      <el-row>
        <el-col :span="10">
          <el-form-item label="暴露的频繁程度" prop="riskDegree">
            <el-select v-model="dataForm.riskDegree" placeholder="暴露的频繁程度" @change="riskDegreeChange">
              <el-option
        ...
        <el-col :span="10">
          <el-form-item label="风险值" prop="riskValue">
            <el-input-number v-model="dataForm.riskValue" :disabled="true" placeholder="风险值"></el-input-number>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="10">
          <el-form-item label="风险级别" prop="staticLevel">
            <el-select v-model="dataForm.staticLevel" placeholder="风险级别" :disabled="true">
              <el-option
...
<script>
  export default {
    ...
    methods: {
          ...
          if (this.dataForm.id) {
            this.$http({
              ...
              if (data && data.code === 0) {
                ...
                this.dataForm.staticLevel = data.enterpriseRiskAssessPoint.staticLevel
                this.dataForm.dynamicLevel = data.enterpriseRiskAssessPoint.dynamicLevel
      ...
      riskProbabilityChange (newValue) {
        this.updateRiskLevel(newValue, "", "");
      },

      riskSeverityChange (newValue) {
        this.updateRiskLevel("", newValue, "");
      },
      riskDegreeChange (newValue) {
        this.updateRiskLevel("", "", newValue);
      },
      updateRiskLevel (riskProbability, riskSeverity, riskDegree) {
        if (riskProbability == null || riskProbability == "") riskProbability = this.dataForm.riskProbability;
        if (riskSeverity == null || riskSeverity == "") riskSeverity = this.dataForm.riskSeverity;
        if (riskDegree == null || riskDegree == "") riskDegree = this.dataForm.riskDegree;

        var riskValue = riskProbability * riskSeverity * riskDegree;
        this.dataForm.riskValue = riskValue;
        if (riskValue < 20) this.dataForm.staticLevel = 5;
        if (riskValue >= 20 && riskValue < 70) this.dataForm.staticLevel = 4;
        if (riskValue >= 70 && riskValue < 160) this.dataForm.staticLevel = 3;
        if (riskValue >= 160 && riskValue < 320) this.dataForm.staticLevel = 2;
        if (riskValue >= 320) this.dataForm.staticLevel = 1;
      },
      // 表单提交
      dataFormSubmit () {
          ...
          if (valid) {
            this.$http({
              ...
              data: this.$http.adornData({
                ...
                'checkPeriod': this.dataForm.checkPeriod,
